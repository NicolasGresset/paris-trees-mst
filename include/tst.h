...
#define tst_prtf(...)                                                          \
  (fprintf(stderr, __VA_ARGS__), tst_zero &= (short)fputc('\n', stderr))
#define tst_prtln(s) fprintf(stderr, "%5d %s", __LINE__, s)

static int tst_prt_results(int fail, int pass, int skip) {
  fprintf(stderr, "%s%d FAIL%s | ", tst_color + tst_str_red, fail,
          tst_str_normal + tst_color);
  fprintf(stderr, "%s%d PASS%s | ", tst_color + tst_str_green, pass,
          tst_str_normal + tst_color);
  fprintf(stderr, "%s%d SKIP%s", tst_color + tst_str_yellow, skip,
          tst_str_normal + tst_color);
  return 0;
}

// The macros `exp1`, `exp2` and `cat2` have been introduced to support
// Micorsoft `cl` compiler
#define tst__count(_1, _2, _3, _4, _5, _6, _7, _8, _9, _A, _B, _C, _D, _E, _F, \
                   _N, ...)                                                    \
  _N
#define tst__argn(...)                                                         \
  tst__count tst__exp1(                                                        \
      (__VA_ARGS__, F, E, D, C, B, A, 9, 8, 7, 6, 5, 4, 3, 2, 1, 0))
...
#define tst_vrg(_f, ...)                                                       \
  tst__exp2(tst__cat2(_f, tst__argn(__VA_ARGS__)), tst__exp1((__VA_ARGS__)))
...
#define tst_tags_5(_0, _1, _2, _3, _4)                                         \
  tst_tags__(4, _1, _2, _3, _4, _5, _6, _7, _8)
#define tst_tags_6(_0, _1, _2, _3, _4, _5)                                     \
  tst_tags__(5, _1, _2, _3, _4, _5, _6, _7, _8)
#define tst_tags_7(_0, _1, _2, _3, _4, _5, _6)                                 \
  tst_tags__(6, _1, _2, _3, _4, _5, _6, _7, _8)
#define tst_tags_8(_0, _1, _2, _3, _4, _5, _6, _7)                             \
  tst_tags__(7, _1, _2, _3, _4, _5, _6, _7, _8)
#define tst_tags_9(_0, _1, _2, _3, _4, _5, _6, _7, _8)                         \
  tst_tags__(8, _1, _2, _3, _4, _5, _6, _7, _8)
...
#define tsttag_2(t_, x_)                                                       \
  ((x_)            ? (tst_tags_val |= (tst_tag_##t_)),                         \
   !(tst_zero = 0) : (tst_tags_val &= ~(tst_tag_##t_)), (tst_zero = 0))
...
#define tst_tags__(n_, _1, _2, _3, _4, _5, _6, _7, _8)                         \
  ...
  static const char *tst_tag_names[8] = {#_1, #_2, #_3, #_4,                   \
                                         #_5, #_6, #_7, #_8};                  \
  static inline int tst_parsetags(int argc, const char **argv) {               \
    return tst_parse_tags(argc, argv, n_, tst_tag_names);                      \
  }                                                                            \
  static inline int tst_tags_zero(void) {                                      \
    return tst_zero &                                                          \
           (tst_tag_##_1 | tst_tag_##_2 | tst_tag_##_3 | tst_tag_##_4 |        \
            tst_tag_##_5 | tst_tag_##_6 | tst_tag_##_7 | tst_tag_##_8);        \
  }

static inline int tst_tags_zero(); // tst_tags_zero() always returns 0 and is
                                   // used just to avoid compiler warnings.

#define TST_STR_HELP_NOTAGS "[--help] [--color] [--report-error] [--list]"
#define TST_STR_HELP_TAGS " [+/-]tag ... ]\ntags:"

static inline short tst_parse_tags(int argc, const char **argv, int ntags,
                                   const char **names) {
  unsigned char v;
  const char *arg;
  ...
  for (int n = 0; n < argc; n++) {
    arg = argv[n];
    while (*arg) { // Allow a single string to contain multiple options (e.g.
                   // "--color +NoDB")
      v = 0xFF;
      if ((arg[0] == '-') && (arg[1] == '-')) {
        switch (arg[2]) {
...
#define tstrun_(tst_, title_, ...)                                             \
  ...
  int main(int argc, char **argv) {                                            \
    ...
    if (CLOCKS_PER_SEC > ((clock_t)1000000) + tst_zero)                        \
      ...
      tst_clock_unit = "m";                                                    \
    fprintf(stderr, "----- %s%s %s \"%s\" %s%s%s\n", tst_color + tst_str_cyan, \
            tst_str_file, __FILE__, tst_title, tst_time(),                     \
            tst_color + tst_str_normal, (tst_ ? "" : " (disabled)"));          \
    if (tst_)                                                                  \
...
static short tst_vars[6] = {
    0}; // Ensures that `tstcheck` can be used outside a `tstcase` block.

// This is only used to avoid that the compiler complains about unused static
// variables.
#define tst_usestatic                                                          \
  ((tst_result & tst_case_pass & tst_case_fail & tst_case_skip & tst_vars[0] & \
    tstdata[0] & (int)tstelapsed))
...
#define tstskipif(tst_)                                                        \
  for (int tst_k = 1; tst_k && ((tst_skip_test = (short)(!!(tst_))), 1);       \
       tst_k = (tst_skip_test =                                                \
                    (short)(tst_skip_test ? (tst_prtln(""),                    \
                                             tst_prtf("%s", tst_str_skip_end)) \
                                          : 0)))                               \
...
#define tstclock(...)                                                          \
  for (clock_t tst_clk = clock(); tst_clk; tstelapsed = (clock() - tst_clk),   \
               tst_prtln(""),                                                  \
               fprintf(stderr, tst_str_clck, tstelapsed, tst_clock_unit),      \
               tst_clk = tst_prtf(__VA_ARGS__))

#define tstnote(...) (tst_prtln(tst_str_note), tst_prtf(" " __VA_ARGS__))

#define tstouterr(...)                                                         \
  for (int tst_k = (tst_prtln(tst_str_scrn), tst_prtf(" " __VA_ARGS__), 1);    \
...
#define tstsection(...)                                                        \
  for (int tst_sect = 1;                                                       \
       ...
       tst_sect = 0, tst_sect_counter = tst_sect_last,                         \
           tst_prtln(tst_str_sctn_end), fputc('\n', stderr))                   \
    for (int tst_data_count = 0; tst_data_count < tst_data_size;               \
         tst_data_count++)
